...
void FreeFloatingFluidPlugin::Update()
{
    ...
    for( auto & link:  buoyant_links_)
    {
        ...
        //ROS_INFO("Link: Applying buoyancy force (%.01f, %.01f, %.01f)", link.buoyant_force.x, link.buoyant_force.y, link.buoyant_force.z);
    }
}
...
void FreeFloatingFluidPlugin::ParseNewModel(const physics::ModelPtr &_model)
{
    ...
    if(!rosnode_->hasParam("/" + description_)){
        return;
    }
    ...
    rosnode_->getParam("/" + description_, urdf_content);
