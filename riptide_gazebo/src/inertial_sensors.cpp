...
InertialSensors::InertialSensors(ros::NodeHandle &nh)
{
  ...
  quatBodyFixedENU2NED_ = auv_core::math_lib::toQuaternion(0, 0, M_PI);  // Roll by 180 deg
...
void InertialSensors::modelStatesCB(const gazebo_msgs::ModelStates::ConstPtr &msg)
{
  ...
  if (!modelFound_)
  {
    ...
    if (!modelFound_)
    {
      ROS_WARN("InertialSensors: Model Name %s not found in topic %s", modelName_.c_str(), modelStatesTopic_.c_str());
      return;
    }
  }
